...
\paragraph{Encoding-level viewpoint (Cubical Agda).}
The entries below differ not only by mathematical motivation but also by type former: predicate records on \(\mathbb{Q}\) (Dedekind/one-sided cuts), sequence-with-modulus records (fast Cauchy), coinductive streams (digit encodings), and higher-inductive quotients (HIT-based completions). In Cubical Agda these encodings have different elimination principles and equality mechanisms, so ``same intended real continuum'' does not imply definitional sameness of types \cite{HoTT2013,Vezzosi2019,Booij2020}.
...
\paragraph{Choice-sensitive boundary.}
Sections 2--3 are read in a base system without countable choice. In this setting, existence statements that are merely propositional do not automatically provide witness-extracting functions, so reverse maps from cut-style encodings to sequence-style encodings are not taken as canonical unless explicitly justified \cite{BishopBridges1985,TroelstraVanDalen1988,Booij2020}.
...
\(\mathbb{R}_\mathrm{CF}\), \(\mathbb{R}_{b}\), \(\mathbb{R}_\mathrm{SD}\), and decimal/base-$10$ variants are concrete stream/fraction presentations. Before quotienting they are representation objects rather than canonical field objects: non-unique encodings and carry/normalization phenomena obstruct direct field laws on raw codes. After quotienting by the appropriate digit/fraction equivalence (with suitable regularity/modulus side conditions), these presentations map into the Cauchy-completion family (Class B) \cite{Weihrauch2000,BoehmEtAl1986,BergerHou2007,Berger2011,Niqui2008,Kaganovsky1998,KoeppSchwichtenberg2022,WiesnetKopp2022,LubarskyRichman2015,BooijLocators,BergerSeisenberger2012,BergerLloyd2009,MathStackConstructiveReals}.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Cauchy/HIT family: \(\RFC\), \(\RI\), \(\RH\), \(\Rinit\), \(\RES\), and \(\RCauComp\) are Cauchy-completion-flavoured presentations. In this document (without \(\mathsf{AC}_\omega\)), we only assert the embeddings/identifications supported by the cited constructions and do not assume a blanket equivalence among all of them in plain Cubical Agda \cite{HoTT2013,Booij2017,Booij2020,EscardoSimpson2001,Murray2022}.
